...
class CheckReturnReminders extends Command
{
    ...
    {
        $orders = Order::needsReminder()->get();
        ...
        foreach ($orders as $order) {
            $daysSince = $order->shipped_at->diffInDays(now());
            ...
            Alert::create([
                ...
                'message' => "Order {$order->order_number} ({$order->product->name}) was shipped {$daysSince} days ago on {$order->platform->name}. Please check delivery status.",
                'entity_type' => 'order',
                'entity_id' => $order->id,
                'is_read' => false,
            ]);
            ...
            $created++;
            $this->line("  ✅ Created alert for {$order->order_number} ({$order->product->name})");
        }
